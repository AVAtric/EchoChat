...
FROM composer:2.3 as composer_base
...
RUN mkdir -p /opt/apps/echo-chat /opt/apps/echo-chat/bin
...
RUN addgroup -S composer \
    ...
    && chown -R composer /opt/apps/echo-chat \
...
COPY --from=composer_base /opt/apps/echo-chat /opt/apps/echo-chat
...
COPY --from=composer_base /opt/apps/echo-chat /opt/apps/echo-chat
COPY --from=frontend /opt/apps/echo-chat/public /opt/apps/echo-chat/public
...
COPY --from=composer_base --chown=www-data /opt/apps/echo-chat /opt/apps/echo-chat
COPY --from=frontend --chown=www-data /opt/apps/echo-chat/public /opt/apps/echo-chat/public
...
# and configuration.
COPY docker/config/nginx.conf.template /etc/nginx/templates/default.conf.template
...
RUN touch laravel.cron && \
    echo "* * * * * cd /opt/apps/echo-chat && php artisan schedule:run" >> laravel.cron && \
